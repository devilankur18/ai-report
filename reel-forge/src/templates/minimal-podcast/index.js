...
import { AbsoluteFill, Sequence, Audio, spring, interpolate, useCurrentFrame, useVideoConfig, staticFile, } from 'remotion';
...
export const MinimalPodcastTemplate = ({ audioUrl, expertName, expertSpecialty, domain, hookText, hookStyle = 'typewriter-bold', hookStat, hookEmoji, toneTag, scenes, ctaText, ctaType, ctaTitle, ctaSubtitle, ctaLink, ctaHandle, accentColor: customAccent, bgGradientStart: customStart, bgGradientEnd: customEnd, bgSolid: customSolid, themeId = 'neon-tech', bgVideoUrl: customBgVideo, language = 'en', expertAvatar, expertLogo, expertImages, expertImageSet, overlayStyle: customOverlayStyle, 
...
clientId, designId, fontPairingIndex: customFontPairingIndex, layoutVariant: customLayoutVariant, decorationStyle: customDecorationStyle, imageTreatment: customImageTreatment, }) => {
    ...
    const resolvedBgImageUrl = React.useMemo(() => {
        if (!isImageBg || allImages.length === 0)
            return undefined;
        if (isCtaActive)
    ...
    return (_jsxs(AbsoluteFill, { children: [_jsx(AnimatedBackground, { bgType: resolvedBgType, bgGradientStart: resolvedBgStart, bgGradientEnd: resolvedBgEnd, bgSolid: resolvedBgSolid, bgVideoUrl: resolvedBgVideo, bgImageUrl: resolvedBgImageUrl, overlayStyle: resolvedOverlayStyle, accentColor: resolvedAccent, imageFilter: resolvedImageFilter }), _jsx(SafeZone, { show: false }), resolvedLogoUrl && frame >= AUDIO_START_FRAME && (_jsx("div", { style: {
                    position: 'absolute', top: '80px', right: '80px',
                    opacity: 0.75, zIndex: 20,
                }, children: _jsx("img", { src: resolvedLogoUrl, alt: "Brand Logo", style: { maxHeight: '50px', maxWidth: '160px', objectFit: 'contain' } }) })), frame < INTRO_DURATION && (_jsx(AbsoluteFill, { style: { zIndex: 30 }, children: _jsx(HookIntro, { hookText: hookText || expertName, hookStyle: hookStyle, hookStat: hookStat, hookEmoji: hookEmoji, toneTag: toneTag, expertName: expertName, expertSpecialty: expertSpecialty, heroImageUrl: hookImage, accentColor: resolvedAccent, textColor: displayTextColor, fonts: fonts, durationFrames: INTRO_DURATION }) })), frame >= AUDIO_START_FRAME && (_jsxs(AbsoluteFill, { style: { opacity: mainContentOpacity, zIndex: 10 }, children: [_jsxs("div", { style: {
                            position: 'absolute',
                            top: '30%',
                            left: '50%',
                            ...
                            alignItems: 'center',
                        }, children: [_jsx(Sequence, { from: AUDIO_START_FRAME, children: _jsx(AudioWaveform, { audioUrl: resolvedAudioUrl, accentColor: resolvedAccent, mode: "radial", numberOfBars: 64 }) }), resolvedAvatarUrl && (_jsx("img", { src: resolvedAvatarUrl, alt: expertName, style: {
                                    position: 'absolute',
                                    width: '320px',
                                    height: '320px',
                                    borderRadius: '50%',
                                ...
                                } }))] }), activeScene?.keyQuote && (_jsx(PersonalizedQuoteCard, { variant: pProfile.layoutVariant, decoration: pProfile.decorationStyle, accentColor: resolvedAccent, textColor: displayTextColor, cardBg: cardBg, cardBorder: cardBorder, textShadow: textShadowValue, fontFamily: fonts.serif, opacity: quoteOpacity, translateY: quoteTranslateY, children: _jsxs("div", { style: { marginTop: '12px' }, children: ["\"", activeScene.keyQuote, "\""] }) })), _jsx(PersonalizedIdentity, { variant: pProfile.layoutVariant, expertName: expertName, expertSpecialty: expertSpecialty, expertAvatar: expertAvatar, accentColor: resolvedAccent, textColor: displayTextColor, textShadow: textShadowValue, fontFamily: fonts.sans, avatarResolver: (u) => {
                            if (!u)
                                return '';
                            if (u.startsWith('http') || u.startsWith('data:') || u.startsWith('/') || u.startsWith('file:'))
                                return u;
