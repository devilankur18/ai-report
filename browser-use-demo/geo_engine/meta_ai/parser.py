...
def extract_entities_from_table(table_lines, citations):
...
def extract_from_blocks(items, citations):
    entities = []
    for item in items:
        if not item.strip():
            continue
        ...
        for cand in candidates:
            if not cand or not is_valid_entity(cand) or not looks_like_provider(cand):
                continue
...
def extract_from_flat_text(ai_answer_text, citations):
    ...
    while i < len(lines):
        ...
        if is_name_line:
            ...
            i += 1
    return extract_from_blocks(candidate_blocks, citations)
...
def parse_meta_ai_logs(input_file, output_json, output_md):
    ...
    if dom_marker in content:
        try:
            ...
            if start_idx != -1:
                decoder = json.JSONDecoder()
                dom_data, _ = decoder.raw_decode(parts[start_idx:])
            else:
    ...
    table_entities = []
    for table in tables:
        table_entities.extend(extract_entities_from_table(table, citations))
        
    # 2. Parse numbered lists
    num_items = re.split(r'\n\s*(?:#+\s*)?(?:\*\*|)\d+[\.\)]\s*(?:\*\*|)?', '\n' + ai_answer_text)
    num_entities = extract_from_blocks(num_items[1:], citations)
    
    # 3. Parse bulleted lists
    bullet_items = re.split(r'\n\s*[-*•]\s+', '\n' + ai_answer_text)
    bullet_entities = extract_from_blocks(bullet_items[1:], citations)
    
    # 4. Parse flat text
    flat_entities = extract_from_flat_text(ai_answer_text, citations)
    
    # Combine all extracted and WS Sniffed entities
    ...
    for ent in merged_entities.values():
        if is_valid_entity(ent["name"]) and looks_like_provider(ent["name"]):
