...
async function run() {
    ...
    const results = await page.evaluate(async () => {
        ...
        const dumpElementsWithText = (text) => {
            const matches = [];
        ...
        logs.push(`Delete matches: ${JSON.stringify(dumpElementsWithText('Delete'))}`);
        logs.push(`Delete Chat matches: ${JSON.stringify(dumpElementsWithText('Delete Chat'))}`);
        ...
        if (moreOptionsBtn) {
            ...
            logs.push(`Delete matches after click: ${JSON.stringify(dumpElementsWithText('Delete'))}`);
            logs.push(`Delete Chat matches after click: ${JSON.stringify(dumpElementsWithText('Delete Chat'))}`);
        } else {
            logs.push("'More options' button not found.");
        }
        
        // Close menu if open (by clicking body or clicking it again)
        document.body.click();
    ...
    console.log("Results:\n", results);
    fs.writeFileSync('scratch/menu_search_results.txt', results, 'utf8');
    await browser.disconnect();
}
