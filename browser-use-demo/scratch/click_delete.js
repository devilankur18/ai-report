    ...
    try {
        ...
        const confirmMatches = await page.evaluate(() => {
            ...
            document.querySelectorAll('*').forEach(el => {
                ...
                const text = (el.innerText || '').trim();
                if ((text.toLowerCase().includes('delete') || text.toLowerCase().includes('cancel')) && text.length < 150) {
                    elements.push({
                        tagName: el.tagName,
                        text: text,
                        class: el.className,
